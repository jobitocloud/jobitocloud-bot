...
export class LibraryFeature {
  ...
  async handleLibFindCommand(message: TelegramBot.Message) {
    const bookToFind = message.text!.replace(COMMAND_LIBFIND, "").trim();
    console.log(message.text, ">>>", bookToFind);
  ...
  async handleLibStudyCommand(message: TelegramBot.Message) {
  ...
  getCommands(): {
    menu: TelegramBot.BotCommand[];
    handlers: CommandHandlerDictionary;
  } {
    return {
      menu: [
        {
      ...
      handlers: {
        "^/libfind": this.handleLibFindCommand.bind(this),
        "^/libstudy": this.handleLibStudyCommand.bind(this),
      },
    };
  }
}
